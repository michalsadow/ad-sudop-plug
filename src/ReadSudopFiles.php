...
use Przeslijmi\AgileData\Operations\Reading\MassReadFromCsv as MyParent;
use Przeslijmi\AgileData\Tools\ConvToDate;
use Przeslijmi\AgileDataSudopPlug\SudopHelper;
use stdClass;

/**
 * Operation that reads SUDOP data from many CSV files at one time.
 */
class ReadSudopFiles extends MyParent implements MyInterface
{
     ...
     * @var string
     */
    protected static $opKey = 'sM49rca6';
    ...
    public static function getInfo(): stdClass
    {
        ...
        // Lvd.
        $locSta = 'Przeslijmi.AgileDataSudopPlug.ReadSudopFiles.';
    ...
    public static function getStepFormFields(string $taskId, ?stdClass $step = null): array
    {
        ...
        $loc    = $_ENV['LOCALE'];
        $locSta = 'Przeslijmi.AgileDataSudopPlug.ReadSudopFiles.fields.';

        // Delete all fields except for dirChooser.
        foreach ($fields as $fieldId => $field) {
            if ($field['type'] !== 'dirChooser') {
